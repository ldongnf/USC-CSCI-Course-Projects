//
//  FavouriteCommitteesViewController.swift
//  Congress
//
...
class FavouriteCommitteesViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate  {
	...
	var favouriteCommitteesSet = [Committee]()
	...
	override func viewWillAppear(_ animated: Bool) {
		favouriteCommitteesSet = FavouriteCommittees.sorted{$0.name < $1.name}
	...
	func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
		filteredCommitteesSet = favouriteCommitteesSet.filter({(committee:Committee) -> Bool in return committee.name.lowercased().range(of: searchText.lowercased()) != nil})
	...
	func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
		if shouldShowSearchResults{
			...
			return favouriteCommitteesSet.count
		}
	}
	...
	func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		...
		if shouldShowSearchResults{
			...
			committee = favouriteCommitteesSet[indexPath.row]
		}
	...
	override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
		...
		if let indexPath = sender as? IndexPath{
			...
			if shouldShowSearchResults{
				...
				currentCommittee = favouriteCommitteesSet[indexPath.row]
